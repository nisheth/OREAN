          ...
          if myaction == 'addUser':
            if form.is_valid():
              ...
              if usrObj.exists():
                 ...
                 oreanURL = request.build_absolute_uri(reverse('home'))
                 msg = """
Hello,

This is an automated message from OREAN. This email was added to an existing project on our system(%s) by %s. You can start exploring the data for this project by creating a OREAN account: 

%s

Thanks,
OREAN Staff 
""" % (request.session['projectName'], request.user.get_full_name(), oreanURL)
                 newInvObj = Invitations(email=email, project=project, user=request.user)
                 newInvObj.save()
                 send_new_email(request, email, "OREAN Invitation", msg)
